...
export default function ProductCard({ product }: ProductCardProps) {
  const { name, imageUrl, price, cashbackPercent, productUrl } = product;
  return (
    <div className="flex flex-col items-center justify-center w-64 p-4 bg-white border border-gray-200 rounded-lg shadow-md">
      <div className="relative w-full pt-[100%] bg-gray-100">
        <Image
          ...
          alt={name}
          fill
          className="object-contain absolute inset-0 p-2"
        />
      </div>
        ...
        <div className="flex items-center space-x-1 mt-2">
          ...
          {productUrl ? (
            <Link
              ...
              className="px-3 py-1 bg-iheart-red text-white text-xs rounded-full"
            >
              Buy Now
            </Link>
          ) : (
